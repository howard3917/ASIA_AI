    ...
    public static class ModelProcessor
    {
        ...
        public static void ProcessModelGroup(ModelGroup group, ref Vector2Int vertCount, ref Vector2Int triCount, ref Vector2Int meshCount, ref Vector2Int boxColliderCount)
        {
                        ...
                        foreach(var decorationSurfaceRenderer in decorationSurfaceRenderers)
                        {
                            if (decorationSurfaceRenderer.gameObject.activeInHierarchy)
                            {
                                if (!decorationSurfacesParent)
                                {
                                    ...
                                    decorationSurfacesParent.transform.SetParent(group.transform, false);
                                }

                                decorationSurfaceRenderer.transform.SetParent(decorationSurfacesParent.transform);
                            }
                        }
                    }
                }
            ...
            if (collapseCols)
            {
                ...
                var partBoxColliders = boxColliders.Where((c) => c.gameObject.layer != LayerMask.NameToLayer(Connection.connectivityReceptorLayerName) && c.gameObject.layer != LayerMask.NameToLayer(Connection.connectivityConnectorLayerName)).ToArray();
            ...
            foreach(var partCollider in partColliders)
            {
                ...
                if (!collidersParent)
                {
                    ...
                    collidersParent.transform.SetParent(group.transform, false);
                }

                partCollider.transform.SetParent(collidersParent.transform);
            }
